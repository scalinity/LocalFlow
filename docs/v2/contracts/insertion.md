...
again: a cancelled or deleted job, or a destination that is no longer
the frontmost application's focused element, stops the effect
(`saved_not_inserted/user_cancelled|job_deleted`,
`target_changed/target_changed_before_effect`). Before the post the
...
| window | `window_element`: the M06 window element (in memory) vs the live field's `AXWindow`; `window_title`: the recorded title vs the live focused-window title (resolved from the application element). Either failing is a change — another window of the same app under an equal title, or another tab in the same window; unreadable ⇒ `unavailable`, identity alone governs | `target_changed` |
...
2. **Terminal destination + text a terminal could act on + no
   ...
   or DEL. The category comes from the snapshot, and when the job
   carries no snapshot the LIVE frontmost bundle is classified. No
...
the settle bound keeps LocalFlow's generation so a late consumer still
pastes the right text (`readback_pending` / `readback_ambiguous`; the
user's clipboard is the disclosed sacrifice). That payload is pending:
...
landed (the user's original is restored first, so the next capture
sees it), when the board moved on (a user copy won), or when its job
was deleted (restored away).
...
— the target consumed part of it; restoring is safe), `mismatch` (a
readback that did not change is pending, never a consumed partial),
...
`undo_last` acts on the original destination only while it is still
...
replacement, empty for a caret insert. Anything else (another field or
...
at the next insertion, at deletion of its job, or at quit; it is never
persisted. **`busy`** reports whether an operation is executing on the
...
neighbourhood with no equal text already there; ambiguity
